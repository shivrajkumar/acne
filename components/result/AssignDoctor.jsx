  ...
  return (
    <>
      {enableFees && (doctor?.fees || doctor?.h1Ttext) && !isDrawer && (
        <p className="flex md:hidden bg-Accent h-[28px] font-lato py-[4px] px-[16px] w-full text-center mb-[16px] uppercase text-[14px] text-Semantic/Success rounded-[8px] justify-center">
      ...
      <div className={`${isSmall ? "max-w-[400px]" : "max-w-full"} ${isSmall ? "md:w-[400px]" : "md:w-full"} sm:w-full bg-Background/Beige border border-[#AFA792] p-[16px] flex flex-col gap-[8px] rounded-[16px]`}>
        {showAssignedDoctorLabel && (
          <p className="font-lato font-[500] text-[14px] text-Text/Heading-Text leading-[1.4] ">
            Assigned doctor
          </p>
        )}
        <div className="flex flex-col md:flex-row gap-[8px] md:gap-[16px]">
          <div className="flex gap-[8px] md:gap-[16px]">
            ...
            <div className="flex flex-col gap-[4px] flex-grow">
              <p className="font-lato font-[600] text-[16px] -tracking-[1%]">{doctor?.name ?? doctor?.doctorName}</p>
              <p className="font-lato text-[12px] md:text-[14px] font-[400]">{doctor?.education ?? doctor?.specialization}</p>
              <p className="font-lato text-[12px] md:text-[14px] font-[400]">{doctor?.experience}</p>
              {enableFees && (doctor?.fees || doctor?.h1Ttext) && (
                <p className="hidden md:flex bg-Accent font-lato py-[4px] px-[16px] w-fit uppercase text-[14px] text-Semantic/Success rounded-[8px] mt-[16px]">
                  {doctor?.fees ?? doctor?.h1Ttext}
                </p>
              )}
              {/* {enableOptIn && (
      ...
      {isDrawer && (doctor?.fees || doctor?.h1Ttext) && (
        <p className="flex md:hidden bg-Accent h-[28px] mt-3 font-lato py-[4px] px-[16px] w-full text-center mb-[16px] uppercase text-[14px] text-Semantic/Success rounded-[8px] justify-center">
          {doctor?.fees ?? doctor?.h1Ttext}
        </p>
      )}
    </>
