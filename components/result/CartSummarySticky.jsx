...
const CartSummarySticky = () => {
    ...
    const { cartDetails: cartData, productsDetails: productDetails, doctorDetails, handleBuyNowClick } = useCartContext()

    // Get first 3 products to display
    const displayProducts = productDetails?.slice(0, 3) || [];

    // Calculate number of remaining products
    const remainingCount = productDetails?.length > 3 ? productDetails.length - 3 : 0;
    ...
    return (
        <>
            <div className="flex justify-between py-[16px] px-[16px] md:py-[16px] md:px-[80px] items-center fixed bottom-0 bg-[#FFFFFF] w-full z-50  shadow-custom-medium">
                <div className="flex gap-[32px]">
                    <div>
                        ...
                        <div className="md:underline font-lato font-[500] text-[14px] -tracking-[2%] text-Neutral/800 flex md:hidden items-center cursor-pointer"
                            onClick={() => setOpen(true)}>
                            Order Summary
                            <span className="ms-[4px]"><Image src={ExpandUp} alt="Expand Up" width={10} height={6} /></span>
                        </div>
                    </div>
                    <div className="hidden md:flex gap-[8px]">
                        {displayProducts.map((product, index) => (
                            <button onClick={scrollToOrderSummary} key={index} className="border-[1px] border-Elements/Divider-Stroke rounded-[8px] p-[4px]" >
                                <Image src={product?.image} alt={product?.name} width={48} height={48} />
                            </button>
                        ))}

                        {remainingCount > 0 && (
                            <button onClick={scrollToOrderSummary} className="flex items-center justify-center border-[1px] border-Elements/Divider-Stroke w-[56px] h-[56px] rounded-[8px] p-[4px]">
                                <span className="font-lato text-[14px] font-[500] text-Neutral/800">+{remainingCount} more</span>
                            </button>
                        )}
                    </div>
                </div>
            ...
            >
                <CartDetails cartData={cartData} productData={productDetails} isMobile={true} doctorDetails={doctorDetails} isSmall={true} isDrawer={true} />
            </Drawer>
        </>
    )
}
